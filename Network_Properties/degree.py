...
def simple(g):
    ...
    # YOUR CODE HERE
    original_pair = g.edges.rdd 
        
    flipped_pair = g.edges.rdd.map(lambda x: [x[1], x[0]])    
    ...
    combined_pairs = original_pair.union(flipped_pair)
            
    dataframe = sqlContext.createDataFrame(combined_pairs, ['src', 'dst']).distinct()
            
    edges = dataframe.filter(dataframe.src != dataframe.dst)
