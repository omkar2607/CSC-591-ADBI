...
def closeness(g):
    ...
    grouped_paths = shortest_paths.select(functions.explode('distances')).groupBy('key')

	# Sum by ID
    sum = grouped_paths.agg(functions.sum('value').alias('s'))

	# Get the inverses and generate desired dataframe.
    result = sum.selectExpr('key as id','1/s as closeness')        
